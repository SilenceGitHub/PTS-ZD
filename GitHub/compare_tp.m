load Data/InitData1_tp_2.mat
data1 = data;
load Data/InitData1_tp_10.mat
data2 = data;
load Data/InitData1_tp_20.mat
data3 = data;
...
fontsize = 20;
figure
plot3(data.desiredPath(1,1:interval1:end),data.desiredPath(2,1:interval1:end),data.desiredPath(3,1:interval1:end),'-','linewidth',3);
hold on;
plot3(data1.actualPath(1,1:interval1:end),data1.actualPath(2,1:interval1:end),data1.actualPath(3,1:interval1:end),'--','linewidth',2);
plot3(data2.actualPath(1,1:interval1:end),data2.actualPath(2,1:interval1:end),data2.actualPath(3,1:interval1:end),'--','linewidth',2);
plot3(data3.actualPath(1,1:interval1:end),data3.actualPath(2,1:interval1:end),data3.actualPath(3,1:interval1:end),'--','linewidth',2);
hold off;
axis equal;
box on;
grid on;
% axis([-1 0 -0.5 0.5 0 0.5]);
view(3);
set(gca,'FontSize',fontsize, 'colororder', all_themes{1}, 'FontName', 'times new Roman');
xlabel('X (m)','fontsize',fontsize,'fontname','times new Roman', 'FontName', 'times new Roman');
ylabel('Y (m)','fontsize',fontsize,'fontname','times new Roman', 'FontName', 'times new Roman');
zlabel('Z (m)','fontsize',fontsize,'fontname','times new Roman', 'FontName', 'times new Roman');
legend('$p_d(t)$','$p_e(t)$ ($t_p= 2$ s)','$p_e(t)$ ($t_p= 10$ s)','$p_e(t)$ ($t_p= 20$ s)','Location','best', 'FontName', 'times new Roman','fontsize',fontsize,'NumColumns',2, 'interpreter','latex');

Error1 = data1.desiredPath - data1.actualPath;
Error2 = data2.desiredPath - data2.actualPath;
Error3 = data3.desiredPath - data3.actualPath;
%
figure;
p1 = plot(t(1:interval:end),Error1(2,1:interval:end)','-.', 'linewidth', 3);
hold on;grid on;
p2 = plot(t(1:interval:end),Error2(2,1:interval:end)','--', 'linewidth', 3);
p3 = plot(t(1:interval:end),Error3(2,1:interval:end)',':', 'linewidth', 3);

legend([p1, p2, p3] ,'$e_{p,y}$  ($t_p= 2$ s)','$e_{p,y}$  ($t_p= 10$ s)','$e_{p,y}$  ($t_p= 20$ s)','Location','best', 'FontName', 'times new Roman','fontsize',fontsize,'interpreter','latex','NumColumns',1);
hold off;
set(gca,'FontSize',fontsize, 'colororder', all_themes{4}, 'FontName', 'times new Roman');
xlabel('$t$ (s)', 'FontName', 'times new Roman','fontsize',fontsize,'interpreter','latex');
ylabel('Error (m)', 'FontName', 'times new Roman','fontsize',fontsize);
