load Data/Data_ts_ptc_step_1e-1.mat
data1 = data;
load Data/Data_ts_ptc_step_1e-2.mat
data2 = data;
load Data/Data_ts_ptc_step_1e-3.mat
data3 = data;
...
[~,col_num] = size(Error1);
for i = 1:col_num
   Error1_norm(i) = norm(Error1(:,i));
end
[~,col_num] = size(Error2);
for i = 1:col_num
   Error2_norm(i) = norm(Error2(:,i));
end
[~,col_num] = size(Error3);
for i = 1:col_num
   Error3_norm(i) = norm(Error3(:,i));
end

figure;
p1 = semilogy(t(1:interval*100:end),Error1_norm(1:interval:end)','-.', 'linewidth', 5);
hold on;grid on;
p2 = semilogy(t(1:interval*100:end),Error2_norm(1:interval*10:end)','--', 'linewidth', 5);
p3 = semilogy(t(1:interval*100:end),Error3_norm(1:interval*100:end)',':', 'linewidth', 5);

legend([p1, p2, p3] ,'$\tau=0.1$','$\tau=0.01$','$\tau=0.001$','Location','best', 'FontName', 'times new Roman','fontsize',fontsize,'interpreter','latex','NumColumns',3);
hold off;
set(gca,'FontSize',fontsize, 'colororder', all_themes{3}, 'FontName', 'times new Roman');
xlabel('$t$ (s)', 'FontName', 'times new Roman','fontsize',fontsize,'interpreter','latex');
ylabel('$\|e(t)\|_2$ (m)', 'FontName', 'times new Roman','fontsize',fontsize,'interpreter','latex');
